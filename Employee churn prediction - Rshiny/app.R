...
server = shinyServer(function(input, output,session){
  ...
  options(shiny.maxRequestSize=30*1024^2) 
   ...
   users <- data.frame(User="churn",Password="employee")
        ...
        if (USER$Logged == TRUE) 
        {
           ...
           ###################################################### ADMIN UI PAGE ###################################################################################################################
                  fluidPage(
                    ...
                    title = tags$div(img(src="log.png","Churn Prediction - Employee", style="margin-top: -4px;margin-left: 30px;", height = 60)),position = "fixed-top",selected = "Upload",inverse = F,
                    tabPanel(title = "Upload",icon = icon("upload"),
                             ...
                             fluidPage(
                               ...
                               column(7,
                                      ...
                                      tags$div(id = 'logo1',img(src="ee.jpg",height='150%',width='85%'),align="center")
                               ),
                               ...
                               column(5,
                                      ...
                                      bootstrapPage(useShinyjs(),
                                                    ...
                                                    tags$h3(strong(em("Predictive Analytics (Employee Churn)")),style="text-align:center;color:#034e91;font-size:190%"),
                                                    br(),
                                                    
                                                    tags$div(id = 'logo2',img(src="c.png",height='60%',width='60%'),align="center"),
    ...
    pred<-reactive({
      ...
      Prediction<-data.frame(ifelse(Churning_Probability>=0.5,"Churn","Not_Churn"))
      final_table<-cbind(S.No=file2[,1],Employee_Name=file2[,2],Prediction=Prediction[,1])
      final_table
    })
    ...
    observeEvent(input$confirmation, {
      if(input$confirmation==TRUE){
        output[["outdata"]]<- renderDataTable({
          datatable(pred(),extensions = c('Scroller'),
                    options = list(
                      dom = 'Bfrtip',
                      deferRender = TRUE,
                      scrollY = 500,
